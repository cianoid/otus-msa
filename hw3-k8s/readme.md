...
- В папке `hw3-k8s/k8s` (если запускаем на ARM) применить команду `kubectl apply -f .` 
(если надо запустить на архитектуре x86, то все запуски производить из папки `hw3-k8s/k8s-amd64`)
